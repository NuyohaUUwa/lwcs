"""Canonical runtime connection flow, packet dispatch, and reconnect control."""
...
if __name__ == "backend.runtime.flow":
    sys.modules.setdefault("game_test.backend.runtime.flow", _CURRENT_MODULE)
elif __name__ == "game_test.backend.runtime.flow":
    sys.modules.setdefault("backend.runtime.flow", _CURRENT_MODULE)
...
def set_auto_reconnect_enabled(enabled: bool) -> dict[str, Any]:
...
def cancel_pending_reconnect(reason: str = "") -> dict[str, Any]:
    session = get_session()
    with session._lock:
...
def _schedule_reconnect(reason: str, *, immediate: bool = False, delay_s: float | None = None):
    ...
    with session._lock:
        ...
        if not _has_reconnect_context(session):
            ...
            return False
        if session.reconnect_state == "running":
            return True
        ...
        session.reconnect_reason = reason
        session.reconnect_last_error = ""
        session.reconnect_next_retry_ts = time.time() + max(0.0, delay_s)
        if delay_s <= 0:
            session.reconnect_banned_until_ts = 0.0
    session.notify_control_state()
    session.notify_status_change()
...
def _control_worker_tick(now: float) -> None:
    ...
    if control_state.get("reconnect_state") in ("scheduled", "banned_wait"):
        ...
        elif next_retry_ts > 0 and now >= next_retry_ts and not session.connected:
    ...
    if (
        ...
        if monster_code:
            ...
            if not res.get("ok"):
                ...
                _emit_control_log(f"后端启动下一轮战斗失败：{error}", level="warn", scope="battle")
...
def login_flow(account: str, password: str, server_name: str) -> dict[str, Any]:
    session = get_session()
...
def fetch_roles_flow(server_ip: str = "", server_port: int = 0, server_name: str = "") -> dict[str, Any]:
    session = get_session()
    if not session.session_id:
        return {"ok": False, "error": "尚未登录，请先执行登录"}
    if server_name and server_name in GAME_SERVERS:
        srv = GAME_SERVERS[server_name]
        server_ip = str(srv["ip"])
        server_port = int(srv["port"])
        session.server_name = server_name
...
def _sync_drain_recv_until_role_stats(*, max_wait_s: float = 15.0) -> dict[str, Any]:
    session = get_session()
    sock = session.sock
    if not sock:
...
def select_role_flow(role_id: str) -> dict[str, Any]:
    session = get_session()
    if not session.sock:
        return {"ok": False, "error": "未连接游戏服，请先获取角色列表"}
    try:
        from game_test.backend.runtime.actions import send_and_wait, send_raw_action
        ...
        if not role_stats_ready:
            ...
            if not drain.get("ok"):
                stop_connection_runtime()
                return {"ok": False, "error": drain.get("error") or "选角成功但未收到角色属性，重连判定失败"}
            with session._lock:
                role_stats_ready = bool(session.role_stats)
        if not role_stats_ready:
            stop_connection_runtime()
