"""Canonical runtime action dispatch and packet send helpers."""
...
if __name__ == "backend.runtime.actions":
    sys.modules.setdefault("game_test.backend.runtime.actions", _CURRENT_MODULE)
elif __name__ == "game_test.backend.runtime.actions":
    sys.modules.setdefault("backend.runtime.actions", _CURRENT_MODULE)

from game_test.backend.domain.combat import battle
from game_test.backend.domain.communication import chat
from game_test.backend.domain.inventory import item_use
from game_test.backend.domain.travel import teleport
...
from game_test.core.connector import send_and_receive_once, send_packet
...
def dispatch_feature_action(action_name: str, payload: dict[str, Any]) -> dict[str, Any]:
    """将动作名映射到具体业务动作。"""
    session = get_session()
    if (
    ...
    if action_name == "item.use":
        ...
        for packet_hex in item_use.build_use_item_packets(item_id, quantity):
            ...
            if res.get("validation_warning"):
                warnings.append(res["validation_warning"])
        session.notify_backpack_update()
        result: dict[str, Any] = {"ok": True, "queued": queued}
        if warnings:
            result["validation_warning"] = " | ".join(dict.fromkeys(warnings))
        return result
...
def send_action(action_name: str, payload: dict[str, Any], *, priority: int = 10, use_queue: bool = True) -> dict[str, Any]:
